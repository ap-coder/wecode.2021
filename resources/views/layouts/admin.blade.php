...
<head>
    ...
    <link href="{{ asset('admin/css/custom.css') }}?dev={{$random}}" rel="stylesheet" /> 
                ...
                <li class="adminbuttonbar">
                    <div class="btn-group">
                        @if(Auth::user()->isAdmin())
                         ...
                         <a type="button"  target="_blank" href="{{ url('/horizon') }}" class="btn btn-outline-success" title="">Horizon</a>
...
<script src="{{ asset('libs/article/lang/en.js') }}"></script>
...
<script>
    ...
    ArticleEditor('.articleeditor-content', {
        ...
        custom: {css: ['{{ asset('libs/article/article-custom.min.css') }}', '{{ asset('libs/css/bootstrap.min.css') }}']},
        grid: {
            classname: 'row',
            columns: 12,
            gutter: '1px',
