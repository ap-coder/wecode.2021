        ...
        <div class="col-md-12">
            @can('staff_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.staff.create') }}">
                            ...
                            <tbody>
                                @foreach($staff as $key => $staff)
                                    <tr data-entry-id="{{ $staff->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $staff->published ?? '' }}</span>
                                        ...
                                        <td>
                                            @can('staff_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.staff.show', $staff->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('staff_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.staff.edit', $staff->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('staff_delete')
                                                <form action="{{ route('frontend.staff.destroy', $staff->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('staff_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.staff.massDestroy') }}",
