    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.content-sections.update", [$contentSection->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="section_title">{{ trans('cruds.contentSection.fields.section_title') }}</label>
                            <input class="form-control" type="text" name="section_title" id="section_title" value="{{ old('section_title', $contentSection->section_title) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="order" id="order" value="{{ old('order', $contentSection->order) }}" step="1">
                            @if($errors->has('order'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('order') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="section" id="section">{{ old('section', $contentSection->section) }}</textarea>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="published" id="published" value="1" {{ $contentSection->published || old('published', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="posts[]" id="posts" multiple>
                                @foreach($posts as $id => $post)
                                    <option value="{{ $id }}" {{ (in_array($id, old('posts', [])) || $contentSection->posts->contains($id)) ? 'selected' : '' }}>{{ $post }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('posts'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="threads[]" id="threads" multiple>
                                @foreach($threads as $id => $thread)
                                    <option value="{{ $id }}" {{ (in_array($id, old('threads', [])) || $contentSection->threads->contains($id)) ? 'selected' : '' }}>{{ $thread }}</option>
                                @endforeach
                            </select>
