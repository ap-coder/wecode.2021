    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-threadsContentSections">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $contentSection->id }}">
                                ...
                                <td>
                                    @can('content_section_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.content-sections.show', $contentSection->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('content_section_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.content-sections.edit', $contentSection->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('content_section_delete')
                                        <form action="{{ route('admin.content-sections.destroy', $contentSection->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-threadsContentSections:not(.ajaxTable)').DataTable({ buttons: dtButtons })
