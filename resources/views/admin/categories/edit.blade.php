        ...
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="published" id="published" value="1" {{ $category->published || old('published', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="published">{{ trans('cruds.category.fields.published') }}</label>
                </div>
            ...
            <label class="w-100 border-bottom mb-3 pb-2">{{ trans('cruds.category.fields.photo') }}</label>
            <div class="featuredimage">
                <input class="form-control" data-toggle="fileupload" data-size="thumbnail" data-button="{{ trans('cruds.category.fields.photo') }}" data-src="false" data-srcid="{{ @get_attachment_url(@$category->photo) }}" data-field="photo" type="hidden" name="photo" value="{{ @$category->photo }}" >
            </div>
            {{-- <div class="form-group">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('homepage') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="homepage" value="0">
                    <input class="form-check-input" type="checkbox" name="homepage" id="homepage" value="1" {{ $category->homepage || old('homepage', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="homepage">{{ trans('cruds.category.fields.homepage') }}</label>
                </div>
