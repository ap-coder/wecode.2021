<div class="m-3">
    @can('video_content_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.video-contents.create') }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-pagesVideoContents">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $videoContent->id }}">
                                ...
                                <td>
                                    @can('video_content_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.video-contents.show', $videoContent->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('video_content_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.video-contents.edit', $videoContent->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('video_content_delete')
                                        <form action="{{ route('admin.video-contents.destroy', $videoContent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('video_content_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.video-contents.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-pagesVideoContents:not(.ajaxTable)').DataTable({ buttons: dtButtons })
