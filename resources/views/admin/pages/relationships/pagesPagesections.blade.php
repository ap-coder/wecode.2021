    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-pagesPagesections">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $pagesection->id }}">
                                ...
                                <td>
                                    @can('pagesection_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.pagesections.show', $pagesection->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('pagesection_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.pagesections.edit', $pagesection->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('pagesection_delete')
                                        <form action="{{ route('admin.pagesections.destroy', $pagesection->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-pagesPagesections:not(.ajaxTable)').DataTable({ buttons: dtButtons })
