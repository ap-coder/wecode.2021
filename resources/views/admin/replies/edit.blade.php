        ...
        <form method="POST" action="{{ route("admin.replies.update", [$reply->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('author') ? 'is-invalid' : '' }}" name="author_id" id="author_id">
                    @foreach($authors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('author_id') ? old('author_id') : $reply->author->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="published" id="published" value="1" {{ $reply->published || old('published', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('private') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="private" id="private" value="1" {{ $reply->private || old('private', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('best_answer') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="best_answer" id="best_answer" value="1" {{ $reply->best_answer || old('best_answer', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('thread') ? 'is-invalid' : '' }}" name="thread_id" id="thread_id">
                    @foreach($threads as $id => $entry)
                        <option value="{{ $id }}" {{ (old('thread_id') ? old('thread_id') : $reply->thread->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @if($errors->has('attachements'))
                    <span class="text-danger">{{ $errors->first('attachements') }}</span>
                @endif
